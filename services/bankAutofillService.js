...
import { CONTENT_FIELDS, acceptedCandidates, contentFingerprint } from './bankContentRules.js';
...
export function getAutofillSettings(env = process.env) {
  return {
    ...
    minimum: boundedNumber(env.BANK_MIN_REMAINING, 30, 90),
...
export async function generateReviewedBatch(client, bank, existing, settings, count) {
  let characters = 0;
...
export async function publishReviewedBatch(spec, batch, run) {
  return sequelize.transaction(async transaction => {
    ...
    for (const item of batch.approved) {
      const fingerprint = contentFingerprint(spec.key, item);
...
export async function maintainBanks(specs, { openai, now = new Date(), settings = getAutofillSettings() } = {}) {
  ...
  for (const spec of Object.values(specs).sort((left, right) => Number(right.key === 'mini_event') - Number(left.key === 'mini_event'))) {
    ...
    try {
      ...
      const batch = await generateReviewedBatch(client, spec.key, supply.existing, settings, Math.min(settings.batchSize, settings.minimum - supply.remaining));
      const published = await publishReviewedBatch(spec, { ...batch, existing: supply.existing }, run);
      results.push({ bank: spec.key, status: 'completed', published, rejected: batch.candidates.length - batch.approved.length });
    } catch (error) {
