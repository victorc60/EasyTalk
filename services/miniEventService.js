    ...
    for (const user of users) {
      try {
        await sendUserMessage(
          bot,
          user.telegram_id,
          `✨ <b>Привет! Готов(а) к теплому субботнему челленджу по английскому?</b>\n\n` +
            `Сегодня у нас мини-игра на 1 день:\n` +
            `• 10 вопросов (грамматика, перевод, выбор правильного варианта)\n` +
            `• Участвуют только те, кто нажал «Участвовать»\n` +
            `• Следующий вопрос приходит только после ответа на текущий\n` +
            `• Базовый интервал около 1 часа, но если вступишь позже — бот ускорит отправку, чтобы успеть до 23:00 (МСК)\n\n` +
            `🏆 <b>Награды:</b>\n` +
            `1 место — 300 очков\n` +
            `2 место — 200 очков\n` +
            `3 место — 100 очков\n` +
            `За участие — 50 очков\n\n` +
            `Внутри игры ты будешь видеть прогресс и текущую позицию в рейтинге.\n\n` +
            `Нажимай кнопку ниже и забирай своё место в игре 🚀`,
          {
            parse_mode: 'HTML',
            reply_markup: {
...
export async function sendMiniEventEntryPoint(bot, chatId, userId) {
  ...
  await sendUserMessage(
    bot,
    chatId,
    `🎯 <b>Субботняя мини-игра по английскому</b>\n\n` +
      `Условия:\n` +
      `• 10 вопросов до 23:00 (МСК)\n` +
      `• Следующий вопрос только после ответа на текущий\n` +
      `• Если вступаешь поздно — интервал автоматически сокращается\n\n` +
      `🏆 Награды:\n` +
      `1 место — 300\n` +
      `2 место — 200\n` +
      `3 место — 100\n` +
      `Участие — 50 очков\n\n` +
      `Нажми «Участвовать», и я сразу запущу твой первый вопрос.`,
    {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [[{ text: '✅ Участвовать', callback_data: `mini_ev_join_${eventKey}` }]]
